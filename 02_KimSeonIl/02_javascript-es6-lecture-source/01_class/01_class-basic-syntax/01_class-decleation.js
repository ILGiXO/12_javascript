...
//JS에서 클래스는  함수의 한 종류이며 프로토타입 기반상속을 더 보기 좋게 만든 구문

class Student {
    //생성자
    //인스턴스를 생성하고 특정 기능을 수행하는 특별한 메서드
    //JS 클래스의 생성자는 한개만 존재할 수 있다.
    constructor(name) {
        this.name = name; // 생성되는 ㅅStudent 객체의 name 프로퍼티에 전달 받은 name을 대입
        this.group = 1;
    }

    // 프로토타입 메서드
    // - 클래스 내부에 정의한 메서드는 자동으로 해당 클래스의 프로토 타입의 프로퍼티로 추가된다.
    introduce(){
...
//  생성자함수는 new 키워드 없이도 사용 가능(단순 함수)
//  Student(); 오류 발생
Teacher();
